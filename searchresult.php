...
if($end==0 && $H>=9){
$q2="SELECT id 'end' FROM `result` WHERE date='$search' order by id desc limit 0,1";
$r2=mysqli_query($cons, $q2);
while($row2=mysqli_fetch_array($r2)){
$end=$row2['end'];	
} 
...
// $sql="SELECT * FROM result WHERE date='$search'  and id between $start and $end order by id ";
...
// $sql="SELECT * FROM result WHERE date='$search'  and id between $start and $end ";
        

        $sql="SELECT * FROM result where date='$search 'order by date DESC ";
        
        // $sql="SELECT * FROM result where date='$search' order by id between $start and $end ";

// $sql="SELECT * FROM result WHERE date='$cdate' and id between $start and $end order by id";

// $sql="SELECT * FROM result WHERE date='$cdate'  and id between $start and $end order by date asc";
        
        // SELECT `dll`, `bha`, `nas`, `dls`, `lott`, `dlm`, `id`, `date` FROM `lotterygame` WHERE 1
...
// $sql="SELECT * FROM result WHERE date='$search'  and id between $start and $end order by id ";
    ...
    <th><?php echo $row['dll'];?> </th> 
    <th>  <?php echo $row['bha'];?></th>
    <th><?php echo $row['nas'];?></th>
    <th><?php echo $row['dls'];?></th>
    <th><?php echo $row['lott'];?></th>
    <th><?php echo $row['dlm'];?></th>
...
<td>'.$row['dlm'].'</td> -->
  ...
  <div class="only">
Purchase of lottery using this website is strictly prohibited in the states where lotteries are banned. You must be above  
18 years to play Online Lottery.
